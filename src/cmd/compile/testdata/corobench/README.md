...
Revision `b2c3584ec4` retains at most four completed, operation-free public
...
20 alternating 500 ms samples at one P produced:

| Probe | Darwin/arm64 parent -> candidate | Linux/amd64 translated parent -> candidate |
| --- | ---: | ---: |
| public yield entry | 1.422 us -> 1.383 us, neutral | 1.044 us -> 933.8 ns (-10.56%, `p=0.023`) |
...
and blocking-C handoff were statistically neutral on both platforms.
`YieldBatch` moved by -9.26% on Darwin and +11.64% under translated Linux. It
enters one public root for the whole benchmark process, so the opposite timing
directions are retained as layout/environment diagnostics rather than a
systematic change. The Linux host reports a VirtualApple CPU; its absolute
timings are directional, while its allocation result independently matches
Darwin.
