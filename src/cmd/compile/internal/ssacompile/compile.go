...
// Compiler satisfies the ssa.Compiler interface.
...
func (c Compiler) Compile(f *ssa.Func, htmlWriter ssa.HTMLWriter) {
	c.compile(f, htmlWriter, nil)
}

// CompileWithPreLowerObserver is like Compile, but calls observer after the
// machine-independent passes and before target-specific lowering. The
// observer cannot replace or stop the native Go backend.
func (c Compiler) CompileWithPreLowerObserver(f *ssa.Func, htmlWriter ssa.HTMLWriter, observer func(*ssa.Func)) {
	c.compile(f, htmlWriter, observer)
}

func (_ Compiler) compile(f *ssa.Func, htmlWriter ssa.HTMLWriter, observer func(*ssa.Func)) {
	...
	for _, p = range passes {
		if p.LoweringStart && observer != nil {
			phaseName = "pre-lower observer"
			observer(f)
		}
