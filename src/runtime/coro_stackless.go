...
// Small frames use eight-element chunks to reduce deep-recursion allocation
// frequency. Larger frames retain four-element chunks so each typed allocation
// remains within the small-object bound after its malloc header. Both sizes fit
// after the six direct markers in the four-bit fused-frame allocation field.
const stacklessCoroFrameChunkSize = 8
const stacklessCoroLargeFrameChunkSize = 4
const stacklessCoroFrameChunkByteLimit = maxSmallSize - mallocHeaderSize
...
const (
	...
	stacklessCoroFrameChunkLast  = stacklessCoroFrameChunkFirst -
		(stacklessCoroFrameChunkSize - 1)
	stacklessCoroShortFrameChunkDirectFirst = stacklessCoroFrameChunkLast - 1
	stacklessCoroShortFrameChunkDirectLast  = stacklessCoroShortFrameChunkDirectFirst -
		stacklessCoroFrameChunkDirectCount
	stacklessCoroShortFrameChunkFirst = stacklessCoroShortFrameChunkDirectLast - 1
	stacklessCoroShortFrameChunkLast  = stacklessCoroShortFrameChunkFirst -
...
const (
	...
	stacklessCoroTaskFusedChunk
	stacklessCoroTaskSwitchPending
	stacklessCoroTaskFusedTransitionMask = stacklessCoroTaskFusedRequested |
		stacklessCoroTaskFusedPending | stacklessCoroTaskFusedChunk
	stacklessCoroTaskCompletionMask = stacklessCoroTaskFusedFrames |
		stacklessCoroTaskFusedTransitionMask | stacklessCoroTaskSwitchPending
)
...
func (s *stacklessCoroScheduler) recycleTaskLocked(task *stacklessCoroTask) {
	...
	if task == &s.root || !validFrame || task.parent != nil ||
		...
		task.flags&stacklessCoroTaskCompletionMask != 0 ||
...
func stacklessCoroFrameChunkLength(size uintptr) uintptr {
	if size <= stacklessCoroFrameChunkByteLimit/stacklessCoroFrameChunkSize {
		return stacklessCoroFrameChunkSize
	}
	return stacklessCoroLargeFrameChunkSize
}

func stacklessCoroFrameChunkBounds(size uintptr) (directFirst, directLast,
	first, last uint16) {
	if stacklessCoroFrameChunkLength(size) == stacklessCoroLargeFrameChunkSize {
		return stacklessCoroShortFrameChunkDirectFirst,
			stacklessCoroShortFrameChunkDirectLast,
			stacklessCoroShortFrameChunkFirst,
			stacklessCoroShortFrameChunkLast
	}
	return stacklessCoroFrameChunkDirectFirst,
		stacklessCoroFrameChunkDirectLast,
		stacklessCoroFrameChunkFirst,
...
func stacklessCoroFusedFrameChunkLastFor(marker uint8) uint8 {
	...
	return stacklessCoroFusedFrameChunkLast
}
...
func isStacklessCoroUncachedFrameLineage(marker uint16) bool {
	return marker == stacklessCoroUncachedFrameLineage ||
		marker <= stacklessCoroFrameChunkDirectFirst &&
			marker >= stacklessCoroShortFrameChunkLast
}
...
func isStacklessCoroFrameChunkElement(marker uint16) bool {
	return (marker <= stacklessCoroFrameChunkFirst &&
		...
		(marker <= stacklessCoroShortFrameChunkFirst &&
			marker >= stacklessCoroShortFrameChunkLast)
}
...
func coroFrameNeedsClear(ctx unsafe.Pointer) bool {
	...
	return task.flags&(stacklessCoroTaskFusedFrames|
		stacklessCoroTaskCacheFrame) != 0 ||
...
// self recursion. chunkType describes the generated typed frame array, so
...
func coroTakeFrameChunk(ctx unsafe.Pointer, child stacklessCoroResume,
	...
	if isStacklessCoroUncachedFrameLineage(parent.frameSize) {
		...
		_, directLast, first, last := stacklessCoroFrameChunkBounds(size)
		switch marker := parent.frameSize; {
		case marker == directLast || marker == last:
			return newobject(chunkType)
		case marker <= first && marker > last:
...
func stacklessCoroFrameChunkClass(chunkType *_type, size uintptr) (uint8, bool) {
	...
	chunkLength := uintptr(array.Len)
	chunkFlag := uint8(0)
	switch chunkLength {
	case stacklessCoroFrameChunkSize:
		if size > stacklessCoroFrameChunkByteLimit/stacklessCoroFrameChunkSize {
			return 0, false
		}
	case stacklessCoroLargeFrameChunkSize:
		chunkFlag = stacklessCoroFusedFrameShortChunk
	default:
		return 0, false
	}
	return chunkFlag, array.Elem != nil && array.Elem.Size_ == size &&
...
func validStacklessCoroFusedFrameMarker(marker uint8) bool {
	...
	return marker&^allowed == 0 &&
		marker&stacklessCoroFusedFrameAllocationMask <=
			stacklessCoroFusedFrameChunkLastFor(marker)
}
...
func coroTakeSelfFrame(ctx unsafe.Pointer, child stacklessCoroResume,
	...
	if parent.state != stacklessCoroTaskRunning || !parent.resuming ||
		parent.readyPending ||
		parent.flags&(stacklessCoroTaskFusedPending|
			stacklessCoroTaskFusedChunk|stacklessCoroTaskSwitchPending) != 0 ||
		parent.terminal != stacklessCoroTerminalNone || parent.goexit {
		unlock(&s.lock)
...
func coroAwaitSelfFrame(ctx, frame unsafe.Pointer,
	...
	s := context.scheduler
	lock(&s.lock)
	pendingFlags := parent.flags & (stacklessCoroTaskFusedPending |
		stacklessCoroTaskFusedChunk)
	if raceenabled || stacklessCoroIsPullComparison(s) ||
		pendingFlags != stacklessCoroTaskFusedPending || parent.resume == nil ||
		stacklessCoroResumeIdentity(parent.resume) !=
			stacklessCoroResumeIdentity(child) {
		unlock(&s.lock)
	...
	if owner != nil {
		...
		marker := parentHeader.marker & stacklessCoroFusedFrameAllocationMask
		chunkLast := stacklessCoroFusedFrameChunkLastFor(parentHeader.marker)
		switch {
		case marker == 0:
...
func coroCompleteSelfFrame(ctx unsafe.Pointer) uint8 {
	...
	if task.state != stacklessCoroTaskRunning || !task.resuming ||
		task.readyPending ||
		task.flags&stacklessCoroTaskCompletionMask !=
			stacklessCoroTaskFusedFrames ||
		task.terminal != stacklessCoroTerminalNone || task.goexit {
		unlock(&s.lock)
...
// compiler-proven structured await. chunkCompatible reports that the parent
// and child generated frame types are identical and may share typed chunks.
// The request is consumed by coroTakeFusedFrame. Race and pull-comparison
// builds retain distinct tasks.
func coroRequestFusedFrame(ctx unsafe.Pointer, chunkCompatible bool) {
	context := (*stacklessCoroContext)(ctx)
	parent := context.task()
	...
	if parent.state != stacklessCoroTaskRunning || !parent.resuming ||
		parent.readyPending ||
		parent.flags&(stacklessCoroTaskFusedTransitionMask|
			stacklessCoroTaskSwitchPending) != 0 ||
		parent.terminal != stacklessCoroTerminalNone || parent.goexit {
		unlock(&s.lock)
		throw("runtime: stackless coroutine fused-frame request outside resume")
	}
	parent.flags |= stacklessCoroTaskFusedRequested |
		stacklessCoroTaskFlags(bool2int(chunkCompatible))*
			stacklessCoroTaskFusedChunk
	unlock(&s.lock)
}
...
// uses the direct prefix and typed frame chunks after the cache fills. Chunk
// compatibility is validated only when no cache-owned holder is available.
func coroTakeFusedFrame(ctx unsafe.Pointer, child stacklessCoroResume,
	...
	requestFlags := parent.flags & (stacklessCoroTaskFusedRequested |
		stacklessCoroTaskFusedChunk)
	requested := requestFlags&stacklessCoroTaskFusedRequested != 0
	requestedChunk := requestFlags&stacklessCoroTaskFusedChunk != 0
	if !requested && requestedChunk {
		unlock(&s.lock)
		throw("runtime: unrequested stackless coroutine fused frame chunk")
	}
	...
	chunkCompatible := sameResume || requestedChunk
	if !chunkCompatible {
		parent.flags |= stacklessCoroTaskFusedPending
		unlock(&s.lock)
		return nil
	...
	pendingFlags := stacklessCoroTaskFusedPending
	if !sameResume {
		pendingFlags |= stacklessCoroTaskFusedChunk
	}
	parent.flags |= pendingFlags
	marker := header.marker & stacklessCoroFusedFrameAllocationMask
	chunkLast := stacklessCoroFusedFrameChunkLastFor(header.marker)
	frame := context.frame
	unlock(&s.lock)
...
func coroAwaitFusedFrame(ctx, frame unsafe.Pointer,
	...
	s := context.scheduler
	lock(&s.lock)
	transitionFlags := parent.flags & stacklessCoroTaskFusedTransitionMask
	if transitionFlags&stacklessCoroTaskFusedRequested != 0 {
		unlock(&s.lock)
		throw("runtime: unconsumed stackless coroutine fused-frame request")
	}
	pendingChunk := transitionFlags&stacklessCoroTaskFusedChunk != 0
	if raceenabled || stacklessCoroIsPullComparison(s) ||
		transitionFlags&stacklessCoroTaskFusedPending == 0 {
		if pendingChunk {
			unlock(&s.lock)
			throw("runtime: unconsumed stackless coroutine fused frame chunk")
		}
		unlock(&s.lock)
		coroAwaitFrame(ctx, frame, child)
		return stacklessCoroActionWait
	...
	sameResume := stacklessCoroResumeIdentity(parent.resume) ==
		stacklessCoroResumeIdentity(child)
	chunkCompatible := sameResume || pendingChunk
	...
	if (chunkCompatible || parentFused) &&
...
func (s *stacklessCoroScheduler) markUncachedFrameLineageLocked(task,
	...
	case marker <= stacklessCoroShortFrameChunkDirectFirst &&
		marker >= stacklessCoroShortFrameChunkDirectLast:
		if marker == stacklessCoroShortFrameChunkDirectLast {
			task.frameSize = stacklessCoroShortFrameChunkFirst
		} else {
			task.frameSize = marker - 1
		}
	case marker <= stacklessCoroShortFrameChunkFirst &&
		marker >= stacklessCoroShortFrameChunkLast:
		if marker == stacklessCoroShortFrameChunkLast {
			task.frameSize = stacklessCoroShortFrameChunkFirst
		} else {
			task.frameSize = marker - 1
		}
...
func (s *stacklessCoroScheduler) readyAfterPanic(task *stacklessCoroTask) {
	...
	task.flags &^= stacklessCoroTaskFusedTransitionMask
